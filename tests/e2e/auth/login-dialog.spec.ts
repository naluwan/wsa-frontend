 ...
 * 2. 未登入狀態下點擊「立刻購買」按鈕會導向建立訂單頁面
 * 3. 未登入狀態下直接訪問受保護頁面會顯示登入對話框
 * 4. 登入對話框中的「前往登入」按鈕可以正確導向登入頁
 *
 * 路由說明：
 * - /courses：新的 Journey-based 課程列表入口頁（主要入口）
 * - /journeys：目前選取課程的章節＋單元總覽頁（依賴 currentCourse context）
 * - /journeys/{slug}/chapters/{chapterId}/missions/{lessonId}：單元詳情頁（播放影片）
 * - /journeys/{slug}/orders：建立訂單＋付款頁面
 */
...
test.describe("登入保護與對話框", () => {
  ...
  test("未登入時點擊「試聽課程」按鈕應顯示登入對話框", async ({ page }) => {
    ...
    const previewButton = page
      .locator('[data-testid="preview-course-button"]')
      .first();
    ...
    if (await previewButton.isVisible()) {
      ...
      await expect(
        page.getByRole("heading", { name: "請先登入" })
      ).toBeVisible();
  ...
  test("未登入時點擊「立刻購買」按鈕應導向建立訂單頁面", async ({ page }) => {
    // Given: 我尚未登入
    // When: 監聽 /api/auth/me 請求以確保登入狀態檢查完成
    const authPromise = page.waitForResponse(
      (response) =>
    ...
    if (await purchaseButton.isVisible()) {
      ...
      // Then: 應該導向到建立訂單頁面（Journey 訂單流程 Step 1）
      await page.waitForURL(/\/journeys\/[^/]+\/orders\?productId=/, {
        timeout: 10000,
      });
      const currentUrl = page.url();
      expect(currentUrl).toMatch(/\/journeys\/[^/]+\/orders\?productId=/);
      console.log("[Test] ✅ 未登入時點擊立刻購買會導向建立訂單頁面");
      console.log(`[Test] 當前 URL: ${currentUrl}`);
    } else {
  ...
  test("登入對話框中的「前往登入」按鈕應正確導向登入頁", async ({ page }) => {
    ...
    if (await previewButton.isVisible()) {
      ...
      await expect(
        page.getByRole("heading", { name: /登入|請選擇登入方式/i })
  ...
  test("未登入時直接訪問受保護的單元頁面應顯示登入提示", async ({ page }) => {
    ...
    await page.goto("/journeys/software-design-pattern/chapters/0/missions/7", {
      waitUntil: "load",
    });
    ...
    const loadingText = await page
      .getByText(/載入中|驗證登入狀態/)
      .isVisible()
      .catch(() => false);
    const loginDialog = await page
      .getByTestId("login-prompt-dialog")
      .isVisible()
      .catch(() => false);
    const loginHeading = await page
      .getByRole("heading", { name: "請先登入" })
      .isVisible()
      .catch(() => false);
    const lockMessage = await page
      .getByText(/您無法觀看|需要購買|無法載入/)
      .isVisible()
      .catch(() => false);

    // 至少應該顯示其中一個狀態
    const hasExpectedState =
      loadingText || loginDialog || loginHeading || lockMessage;
  ...
  test("未登入時訪問免費試看單元應顯示登入對話框但可看到影片", async ({
    page,
  }) => {
    // Given: 我尚未登入
